...
class DisasterEvaluator:
    ...
    def evaluate(self) -> Dict:
        ...
        metrics = {
            ...
            'f1': 2 * (results.results_dict['metrics/precision(B)'] * 
                      results.results_dict['metrics/recall(B)']) / 
                  (results.results_dict['metrics/precision(B)'] + 
                   results.results_dict['metrics/recall(B)'] + 1e-10),
        }
        ...
        try:
            ...
            metrics['per_class_ap50'] = {
                self.CLASS_NAMES[i]: per_class_metrics[i] 
...
def parse_args():
    ...
    parser.add_argument(
        '--save-dir', '-p',
        type=str,
        default='models/val_results',
        help='结果保存目录 (项目内相对路径)'
    )
    parser.add_argument(
        '--save-report',
        type=str,
        default='models/eval_report.txt',
        help='评估报告保存路径 (项目内)'
    )
