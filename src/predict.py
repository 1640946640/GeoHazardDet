...
class DisasterPredictor:
    ...
    def _draw_bbox(self, image: np.ndarray, box):
        ...
        # 计算标签背景
        (text_width, text_height), _baseline = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 2)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="泥石流/滑坡地质灾害检测推理")

    # 模型配置
    parser.add_argument(
        "--weights", "-w", type=str, default=r"models/weights/best.pt", help="模型权重文件路径 (默认使用项目内best.pt)"
    )
    ...
    # 推理参数
    parser.add_argument("--conf-thres", "-conf", type=float, default=0.25, help="置信度阈值")
    parser.add_argument("--iou-thres", "-iou", type=float, default=0.45, help="NMS的IoU阈值")
    parser.add_argument("--imgsz", "-img", type=int, default=640, help="输入图片尺寸")
    parser.add_argument("--device", "-dev", type=str, default="0", help="推理设备 (GPU编号或cpu)")

    # 输出配置
    parser.add_argument("--save-dir", type=str, default="models/predict_results", help="结果保存目录 (项目内相对路径)")
    parser.add_argument("--output", "-o", type=str, default=None, help="输出文件路径 (视频/图片)")

    # 推理模式
    parser.add_argument("--mode", "-m", type=str, default="auto", help="推理模式 (image/video/camera/batch/auto)")
...
def main():
    ...
    if args.mode == "image" or (
        args.mode == "auto"
        and source_path.is_file()
        and source_path.suffix.lower() in [".jpg", ".jpeg", ".png", ".bmp", ".tif", ".tiff"]
    ):
        # 单张图片
    ...
    elif args.mode == "video" or (
        args.mode == "auto" and source_path.suffix.lower() in [".mp4", ".avi", ".mov", ".mkv"]
    ):
        # 视频
