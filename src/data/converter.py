...
class ShpToYoloConverter:
    ...
    用于将地理信息系统（GIS）中的Shapefile矢量数据 转换为YOLO目标检测格式的标注文件
    ...
    def read_shapefile(self, shp_path: Path) -> tuple[list, dict]:
        ...
        if shapefile is None:
            raise ImportError("pyshp库未安装，请先安装: pip install pyshp")
    ...
    def extract_yolo_labels(self, records_and_shapes: list[tuple]) -> list[tuple[int, float, float, float, float]]:
    ...
    def _identify_disaster_type(self, record) -> str | None:
        """识别灾害类型.
    ...
    def _shape_to_bbox(self, shape) -> tuple[float, float, float, float] | None:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Shapefile转YOLO格式转换器")
    parser.add_argument("--shp-dir", type=str, default="datasets/全国地质灾害shp/", help="Shapefile目录路径")
    parser.add_argument("--output-dir", type=str, default="datasets/", help="输出目录路径")
